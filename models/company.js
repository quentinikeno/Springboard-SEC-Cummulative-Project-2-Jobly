...
class Company {
	...
	/** Find all companies based on filter.
	 * Accepts object of filters, where filters can be { nameLike, minEmployees, maxEmployees }
	 *
	 * Returns [{ handle, name, description, numEmployees, logoUrl }, ...]
	 * */
	...
	static async findAllFiltered(filters) {
		...
		const conds = keys.map((condName, idx) => {
			if (condName === "nameLike") {
				return `name ILIKE $${idx + 1}`;
			} else if (condName === "minEmployees") {
				return `num_employees >= $${idx + 1}`;
			} else if (condName === "maxEmployees") {
				return `num_employees <= $${idx + 1}`;
			}
		});
